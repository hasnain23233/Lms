...
export default function AllCourses() {
    ...
    const { enrollments, enrollCourse, fetchEnrollments } = useEnrollmentStore();
    ...
    useEffect(() => {
        fetchCourses();
        if (user) {
            fetchEnrollments(); // ✅ get enrolled courses for logged-in user
        }
    }, [fetchCourses, fetchEnrollments, user]);

    // ✅ Timer for guest user (not logged in at all)
    useEffect(() => {
        if (!user) {
            const timers = {};
    ...
    const isEnrolled = (courseId) => {
        return enrollments.some((enroll) => enroll._id === courseId);
    };
                                ...
                                {course.youtubeLink && (
                                    <div className="mb-4 relative">
                                        <iframe
                                            ...
                                            src={
                                                enrolled
                                                    ? getEmbedUrl(course.youtubeLink) // ✅ full video
                                                    : `${getEmbedUrl(course.youtubeLink)}?start=0&end=60` // ✅ 1 min only
                                            }
                                            title={course.title}
                                            frameBorder="0"
